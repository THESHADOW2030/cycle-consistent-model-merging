...
from .conversion import FM_to_p2p, p2p_to_FM
...
def zoomout_refine(
    FM_12, evects1, evects2, nit=10, step=1, A2=None, subsample=None, return_p2p=False, n_jobs=1, verbose=False
):
    """
    ...
    for it in iterable:
        if use_subsample:
            FM_12_zo = zoomout_iteration(FM_12_zo, evects1[sub1], evects2[sub2], A2=None, step=step, n_jobs=n_jobs)

        else:
            FM_12_zo = zoomout_iteration(FM_12_zo, evects1, evects2, A2=A2, step=step, n_jobs=n_jobs)
...
def graph_zoomout_refine(
    FM_12, evects1, evects2, G1=None, G2=None, nit=10, step=1, subsample=None, return_p2p=False, C=1, verbose=False
):
    """
    ...
    _FM_zo = zoomout_refine(
        FM_12, evects1, evects2, nit, step=step, subsample=sub, return_p2p=False, n_jobs=1, verbose=verbose
    )
